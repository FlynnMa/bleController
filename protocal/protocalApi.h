 ...
 */
typedef int32_t (*pfnSendData)(void* pBuf, uint32_t len);
 ...
 * @param[i]  scheduleFunc   this function is used when an asynchoronous event
 *                         callback is requested, the application needs to
 *                         dispatch and handle the event in an individual thread
 *                         or task.
 *
 * @return @ERROR_TYPE
 */
 ...
 *
 */
void protocalApiRouter(uint8_t ch);
