...
/*!
 * @brief this function returns remaining space of list
 *
 * @param[i] pList   an instance of list_t
 * @param[o] pRemain remaining count of list_t
 *
 * returns error codes
 */
int listRemain(list_t *pList, uint32_t *pRemain)
{
    if (NULL == pList || (NULL == pRemain)) {
        return -1;
    }

    *pRemain = MAX_LIST_SIZE - pList->count;
    return 0;
}
